...
```{r}
df_vac_counties <- data_vac %>% count(FIPS, Recip_County)

df_vac_counties %>% 
  anti_join(data_pres, by = 'FIPS') #tie kuriu truksta kitam datasete
```
...
data_vac_latest <- data_vac %>%
  group_by(FIPS)... #is ankstesnes paskaitos
```
